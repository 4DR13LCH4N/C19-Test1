...
function setup() {
 ...
 console.log(message)
  ...
  frisk.addAnimation("running", frisk_running);
  frisk.addAnimation("collided", frisk_collided);
  frisk.scale = 0.5;

  sans = createSprite(50, 160, 20, 50);
  sans.addAnimation("running2", sans_running);
  sans.addAnimation("collided2", sans_still);
...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    if(bonesGroup.isTouching(frisk)){
      ...
      frisk_collided.visible = true;
     
     //change the frisk animation
      frisk.changeAnimation("collided", frisk_collided);

      if(sans.isTouching(frisk || frisk_collided || frisk_running)) {
        sans.velocityX = 0;
        sans.velocityY = 0;
        sans.changeAnimation("collided2", sansStill);
      }
...
function reset(){
  ...
  frisk.changeAnimation("running", frisk_running);
  sans.changeAnimation("running2", sans_running);
  score = 0;
}
